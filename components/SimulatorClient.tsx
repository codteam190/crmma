...
import { Calculator, Settings, Box, TrendingUp, PieChart, FileText } from 'lucide-react';

export default function SimulatorClient() {
  // ================= STATES (L-Modkhalat) =================
  const [country, setCountry] = useState<string>('LB'); // LB awla MA
  ...
  const [exchangeRate, setExchangeRate] = useState<number>(10); // Khass b Morocco

  // Lebanon Specific
  const [codFeePercent, setCodFeePercent] = useState<number>(3); 
  const [deliveryCostUSD, setDeliveryCostUSD] = useState<number>(9); 

  // Morocco Specific (B d-Dirham MAD)
  ...
  // 🧠 L-Math 3la 7ssab d-Dwla:
  let totalDelivery = 0;
  let codFees = 0;
  let callCenterUSD = 0;
  let fulfillmentUSD = 0;
  let otherFeesTotal = 0; // Hedha ghir bach njm3ouh f Financial Summary

  if (country === 'LB') {
    totalDelivery = deliveredOrders * deliveryCostUSD;
    codFees = totalRevenue * (codFeePercent / 100);
    otherFeesTotal = codFees;
  } else if (country === 'MA') {
    const safeExchangeRate = exchangeRate > 0 ? exchangeRate : 1;
    totalDelivery = deliveredOrders * (deliveryCostMAD / safeExchangeRate);
    ...
    otherFeesTotal = callCenterUSD + fulfillmentUSD;
  }

  const totalCosts = adSpend + totalProductCost + totalDelivery + otherFeesTotal;
  ...
  const p3 = p2 + ((totalDelivery / safeTotalCosts) * 100);
  ...
  let donutBackground = "";
  if (country === 'LB') {
    p4 = p3 + ((codFees / safeTotalCosts) * 100);
    donutBackground = `conic-gradient(#ef4444 0% ${p1}%, #10b981 ${p1}% ${p2}%, #3b82f6 ${p2}% ${p3}%, #f59e0b ${p3}% ${p4}%)`;
  } else {
    p4 = p3 + ((callCenterUSD / safeTotalCosts) * 100);
    p5 = p4 + ((fulfillmentUSD / safeTotalCosts) * 100);
    // Red, Green, Blue, Purple (Call Center), Cyan (Fulfillment)
  ...
  const resetDefaults = () => {
    ...
    setCodFeePercent(3); setDeliveryCostUSD(9);
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 p-4 sm:p-8 font-sans">
      ...
      <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        ...
        {/* Dropdown & Reset: Style jdida kima tlbti (Pill style) */}
        <div className="flex items-center gap-3">
          <select 
            value={country} 
            onChange={(e) => setCountry(e.target.value)} 
            className="px-4 py-2 bg-white border border-gray-200 rounded-full text-sm font-medium text-gray-700 outline-none shadow-sm cursor-pointer hover:bg-gray-50 transition-colors"
          >
            <option value="MA">MA Morocco (MAD)</option>
            <option value="LB">LB Lebanon (USD)</option>
          </select>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            ...
            <div className="space-y-5">
              ...
              {/* Exchange Rate (Kay-ban ghir f Morocco) - N9inaha kima l-khrin */}
              {country === 'MA' && (
                <div className="pt-2">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 min-h-[140px]">
            ...
            <p className="text-xs text-gray-500 mb-4">
              Shipping and processing costs. {country === 'MA' ? <span className="font-bold text-[#10b981]">(In MAD)</span> : <span className="font-bold text-[#3b82f6]">(In USD)</span>}
            </p>
            ...
            {country === 'LB' && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 animate-in fade-in zoom-in duration-300">
                <div>
                  <label className="block text-xs font-bold text-gray-700 mb-1.5">COD Fees (%)</label>
                  <div className="relative">
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 font-bold">%</span>
                    <input type="number" min="0" step="0.1" value={codFeePercent} onChange={(e) => setCodFeePercent(Number(e.target.value))} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm font-bold outline-none focus:border-blue-500 focus:bg-white transition-colors" />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-700 mb-1.5">Delivery Cost</label>
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 font-bold">$</span>
                    <input type="number" min="0" step="0.01" value={deliveryCostUSD} onChange={(e) => setDeliveryCostUSD(Number(e.target.value))} className="w-full pl-7 pr-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm font-bold outline-none focus:border-blue-500 focus:bg-white transition-colors" />
                  </div>
                </div>
              </div>
            )}
            ...
            {country === 'MA' && (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 animate-in fade-in zoom-in duration-300">
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-700 mb-1.5">Call Center (Per Delivered)</label>
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 font-bold text-xs">MAD</span>
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-700 mb-1.5">Fulfillment</label>
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 font-bold text-xs">MAD</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div className="bg-[#3b82f6] rounded-2xl p-6 shadow-md text-white relative overflow-hidden">
              <div className="absolute -right-4 -bottom-4 opacity-10"><Calculator className="w-32 h-32" /></div>
              <p className="text-xs font-bold uppercase tracking-wider mb-2 opacity-90">Total Net Profit</p>
              <h3 className="text-4xl font-black">${netProfit.toFixed(2)}</h3>
            </div>
            
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex flex-col justify-center">
              <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 text-center">Profit Per Order</p>
              <h3 className={`text-3xl font-black text-center ${profitPerOrder >= 0 ? 'text-[#10b981]' : 'text-red-500'}`}>
                ${profitPerOrder.toFixed(2)}
              </h3>
            </div>

            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex flex-col justify-center">
              <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 text-center">ROI</p>
              <h3 className={`text-3xl font-black text-center ${roi >= 0 ? 'text-[#10b981]' : 'text-red-500'}`}>
                {roi.toFixed(1)}%
              </h3>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center justify-center relative">
              ...
              {/* Legend: Updated to show precise Moroccan breakdown */}
              <div className="flex flex-wrap justify-center gap-x-4 gap-y-2 text-xs font-bold text-gray-600">
                ...
                <div className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-[#3b82f6]"></span>Delivery Fees</div>
                {country === 'LB' ? (
                  <div className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-[#f59e0b]"></span>COD Fees</div>
                ) : (
                  <>
              ...
              <div className="space-y-3 text-sm">
                ...
                {country === 'LB' ? (
                  <div className="flex justify-between text-[#ef4444] pb-3 border-b border-gray-100">
                    <span>COD Fees ({codFeePercent}%)</span>
